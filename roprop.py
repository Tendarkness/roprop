...
# A pwntools/capstone disassembly line looks like:
#
#     0:   48 be 00 20 40 00 00 00 00 00   movabs rsi, 0x402000
#     ^offset  ^hex column                 ^mnemonic
#
# Only the hex column is recoloured, so the padding capstone computed for
# the mnemonic column survives untouched. The gap must be 2+ spaces (or the
# end of the line) so a mnemonic that happens to look like hex — "db", for
# instance — is never swallowed into the byte column.
_DISASM_LINE_RE = re.compile(
    r"^(\s*[0-9a-f]+:\s+)"              # offset + separator
    r"((?:[0-9a-f]{2} )*[0-9a-f]{2})"   # hex bytes
    r"(\s{2,}|\s*$)"                    # gap
    r"(.*)$",                           # mnemonic + operands
    re.IGNORECASE,
)
...
def _hilite_disasm_line(line: str, badchars: bytes) -> str:
    """Re-emit one disassembly line with bad bytes flagged in red."""
    match = _DISASM_LINE_RE.match(line)
    if not match or not badchars:
        return f"{CYAN}{line}{RESET}"

    head, hex_col, gap, mnemonic = match.groups()
    cells = []
    for cell in hex_col.split():
        try:
            value = int(cell, 16)
        except ValueError:
            cells.append(cell)
            continue
        cells.append(f"{RED}{BOLD}{cell}{RESET}{CYAN}"
                     if value in badchars else cell)
    return f"{CYAN}{head}{' '.join(cells)}{gap}{mnemonic}{RESET}"
